        ...
        <div class="bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden">
            <div class="p-6">
                ...
                <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
                    <div>
                        <h1 class="text-3xl font-bold bg-gradient-to-r from-slate-800 to-indigo-600 bg-clip-text text-transparent mb-2">
                            Participants - {{ $event->title }}
                        </h1>
                        <p class="text-lg text-slate-600">Gestion des inscriptions et validation des billets</p>
                    </div>
                    <div class="flex flex-wrap items-center gap-3">
                        <a href="{{ route('events.show', $event) }}"
                           class="inline-flex items-center px-4 py-3 bg-white border border-slate-200 text-slate-800 font-semibold rounded-xl shadow-lg hover:bg-slate-50 transition-all duration-200">
                ...
                <!-- Filtres -->
                <div class="mb-8">
                    <form method="GET" action="{{ route('events.attendees', $event) }}" class="bg-white rounded-2xl border border-slate-200 p-6 shadow-lg overflow-x-auto overflow-hidden">
                        <div class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end ">
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-2">Statut paiement</label>
                                <select name="status" class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-2">Méthode</label>
                                <select name="method" class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm">
                            ...
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-2">Du</label>
                                <input type="date" name="from" value="{{ $filters['from'] ?? '' }}" class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-slate-700 mb-2">Au</label>
                                <input type="date" name="to" value="{{ $filters['to'] ?? '' }}" class="w-full rounded-xl border-slate-300 focus:border-indigo-500 focus:ring-indigo-500 shadow-sm" />
                            </div>
                            <div class="flex gap-3">
                                <button type="submit" class="inline-flex items-center px-4 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold rounded-xl hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 shadow-lg hover:shadow-xl">Filtrer</button>
                                <a href="{{ route('events.attendees', $event) }}" class="inline-flex items-center px-4 py-3 bg-white border border-slate-300 text-slate-700 font-semibold rounded-xl hover:bg-slate-50 transition-all duration-200 shadow-sm hover:shadow-md">Réinitialiser</a>
                            </div>
                        </div>
                    </form>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div class="dashboard-card bg-gradient-to-br from-cyan-500 to-blue-500 rounded-2xl p-6 shadow-xl text-white relative overflow-hidden">
                        <div class="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full -translate-y-12 translate-x-12"></div>
                        <div class="relative z-10">
                            <div class="flex items-center justify-between">
                                <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
                    ...
                    <div class="dashboard-card bg-gradient-to-br from-rose-500 to-pink-500 rounded-2xl p-6 shadow-xl text-white relative overflow-hidden">
                        <div class="absolute top-0 right-0 w-20 h-20 bg-white/10 rounded-full -translate-y-10 translate-x-10"></div>
                        ...
                        <div class="relative z-10">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-2xl font-bold">{{ \App\Support\Currency::format($rev, $currency) }}</p>
                                </div>
                                <div class="p-3 bg-white/20 rounded-xl">
                                    <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 1.119-3 2.5S10.343 13 12 13s3 1.119 3 2.5S13.657 18 12 18m0-10V6m0 12v-2m8-4a8 8 0 11-16 0 8 8 0 0116 0z"/></svg>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-slate-200">
                            <thead class="bg-slate-50/80">
                                <tr>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Participant</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Date d'inscription</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Statut paiement</th>
                                    <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-700 uppercase tracking-wider">Validation</th>
                                    <th scope="col" class="px-6 py-4 text-right text-xs font-semibold text-slate-700 uppercase tracking-wider">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="bg-white divide-y divide-slate-200">
                                @forelse($attendees as $registration)
                                    <tr class="hover:bg-slate-50/50 transition-colors duration-150 {{ $registration->is_validated ? 'bg-emerald-50/30' : '' }}">
                                        <!-- Participant -->
                                        <td class="px-6 py-5 whitespace-nowrap">
                                            <div class="flex items-center">
                                                <div class="flex-shrink-0 h-12 w-12 bg-gradient-to-br from-indigo-100 to-purple-100 rounded-2xl flex items-center justify-center shadow-sm">
                                        ...
                                        <td class="px-6 py-5 whitespace-nowrap">
                                            ...
                                            @if($ps === 'paid')
                                                <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 border border-emerald-200 shadow-sm">
                                            ...
                                            @elseif($ps === 'unpaid')
                                                <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-800 border border-amber-200 shadow-sm">
                                                    <svg class="w-3 h-3 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            @elseif($ps === 'pending')
                                                <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-orange-100 text-orange-800 border border-orange-200 shadow-sm">
                                                    <svg class="w-3 h-3 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            @elseif($ps === 'failed')
                                                <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-red-100 text-red-800 border border-red-200 shadow-sm">
                                                    <svg class="w-3 h-3 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <td class="px-6 py-5 whitespace-nowrap">
                                            ...
                                            @else
                                                <span class="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-semibold bg-slate-100 text-slate-800 border border-slate-200 shadow-sm">
                                                    <svg class="w-3 h-3 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                                    </svg>
                                                    En attente
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center">
                                                ...
                                                <p class="text-base font-semibold text-slate-700">Aucun participant pour le moment</p>
                                                <p class="text-sm text-slate-500 mt-1">Les participants apparaîtront ici une fois inscrits</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
                ...
                @if($attendees->hasPages())
                    <div class="mt-8 bg-white/50 backdrop-blur-sm rounded-xl p-6 border border-slate-100">
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const renderAlert = (payload) => {
            ...
            wrapper.className = 'rounded-2xl border border-indigo-200 bg-gradient-to-r from-indigo-50 to-blue-50 px-6 py-4 shadow-xl flex items-start gap-4 animate-fade-in';
            wrapper.innerHTML = `
