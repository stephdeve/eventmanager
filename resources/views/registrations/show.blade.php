                            ...
                            <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach($registration->tickets as $ticket)
                                    <div class="border rounded-lg p-4 space-y-3">
                                        ...
                                        @if($registration->event && $registration->event->allow_ticket_transfer && $ticket->status === 'valid' && (!$registration->event->end_date || now()->lt($registration->event->end_date)) && ((int) $ticket->owner_user_id === (int) $registration->user_id))
                                            <div id="transfer-form-{{ $ticket->id }}" class="mt-2 w-full hidden bg-indigo-50 border border-indigo-200 rounded-lg p-3">
                                                <form method="POST" action="{{ route('tickets.transfer', $ticket->qr_code_data) }}" class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                                    @csrf
                                                    <div class="sm:col-span-2">
                                                        <label for="recipient_email_{{ $ticket->id }}" class="sr-only">Email du destinataire</label>
                                                        <input type="email" required name="recipient_email" id="recipient_email_{{ $ticket->id }}" placeholder="destinataire@example.com" class="w-full rounded-md border-gray-300 focus:ring-indigo-500 focus:border-indigo-500 px-3 py-2" />
                                                    </div>
                                                    <div class="flex items-center gap-2 flex-wrap">
                                                        <button type="submit" class="inline-flex items-center justify-center px-4 py-2 w-full sm:w-auto min-w-[180px] bg-indigo-600 text-white text-sm font-medium rounded-md hover:bg-indigo-700">
                                                            Confirmer le transfert
                                                        </button>
                                                        <button type="button" onclick="toggleTransfer({{ $ticket->id }})" class="inline-flex items-center justify-center px-4 py-2 w-full sm:w-auto border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                                                            Annuler
                                                        </button>
                                                    </div>
                                                </form>
                                                <p class="mt-2 text-xs text-gray-600">Action irréversible: un nouveau QR sera généré et vous perdrez l'accès à ce ticket.</p>
                                            </div>
                                        @endif
                                    </div>
                                @endforeach
