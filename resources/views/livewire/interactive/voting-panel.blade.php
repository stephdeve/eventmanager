<div x-data="buyCoinsState({
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8 p-6 bg-gradient-to-r from-gray-50 to-blue-50 rounded-2xl border border-gray-200">
        <div class="flex items-center gap-4">
            <div class="w-12 h-12 bg-gradient-to-r from-yellow-400 to-yellow-500 rounded-xl flex items-center justify-center shadow-lg">
            ...
            <div>
                ...
                <div class="text-xs text-gray-500 mt-1">1 coin = {{ \App\Models\Setting::get('interactive.coin_price_minor', 100) / 100 }} F CFA</div>
            </div>
        </div>
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <button type="button" wire:click="$refresh"
                    class="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-300 text-gray-700 text-sm font-semibold rounded-xl hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-all duration-200">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($event->participants as $participant)
            <div class="bg-white rounded-2xl border border-gray-200 p-6 shadow-sm hover:shadow-md transition-all duration-200 hover:-translate-y-1">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex-1 min-w-0">
                        <h4 class="font-semibold text-gray-900 text-sm truncate">{{ $participant->name }}</h4>
                        @if($participant->country)
                            <div class="flex items-center gap-1 mt-1">
                                <svg class="w-3 h-3 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3.055 11H5a2 2 0 012 2v1a2 2 0 002 2 2 2 0 012 2v2.945M8 3.935V5.5A2.5 2.5 0 0010.5 8h.5a2 2 0 012 2 2 2 0 104 0 2 2 0 012-2h1.064M15 20.488V18a2 2 0 012-2h3.064M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>
                                </svg>
                                <span class="text-xs text-gray-500">{{ $participant->country }}</span>
                            </div>
                        @endif
                    </div>
                    <div class="text-right">
                        <div class="text-xs text-gray-500 mb-1">Score total</div>
                        <div class="text-lg font-bold text-gray-900">{{ $participant->score_total }}</div>
                    </div>
                </div>

                <div class="flex gap-2">
                    <button wire:click="freeVote({{ $participant->id }})"
                            class="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 bg-gradient-to-r from-indigo-600 to-purple-600 text-white text-xs font-medium rounded-lg hover:from-indigo-700 hover:to-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200">
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="form-group">
                    ...
                    <input type="number" min="10" step="1" x-model.number="qty"
                           class="form-input @error('qty') error @enderror"
                ...
                <div class="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-4 border border-blue-200">
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <span class="text-lg font-bold text-blue-700" x-text="(qty && qty>=min ? (qty*unit/100).toLocaleString('fr-FR') : '0') + ' ' + currency"></span>
                    </div>
                    <div class="text-xs text-gray-500 mt-1">
                        <span x-text="qty || 0"></span> coins × <span x-text="(unit/100).toLocaleString('fr-FR')"></span> <span x-text="currency"></span> par coin
                    </div>
                </div>
            </div>
    ...
    <script>
        document.addEventListener('livewire:init', () => {
            window.addEventListener('toast', (e) => {
                ...
                toast.className = 'fixed z-60 top-4 right-4 max-w-sm px-4 py-3 rounded-xl shadow-lg text-white transform transition-all duration-300 ' +
                    ...
                    try {
                        ...
                        if (!data || !data.success || data.provider !== 'kkiapay') {
                            window.dispatchEvent(new CustomEvent('toast', {
                                detail: { type: 'error', message: 'Paiement indisponible pour le moment.' }
                            }));
                            return;
                        }
                        ...
                        function onSuccess(resp) {
                            try {
                                const transactionId = resp?.transactionId || resp?.transaction_id || resp?.reference || resp?.id;
                                ...
                                .then(response => {
                                    if (response?.success) {
                                        ...
                                        window.dispatchEvent(new CustomEvent('toast', {
                                            detail: { type: 'success', message: 'Coins crédités avec succès !' }
                                        }));
                                        self.openBuy = false;
                                        self.qty = 10;
                                    } else {
                                        window.dispatchEvent(new CustomEvent('toast', {
                                            detail: { type: 'error', message: response?.message || 'Erreur de confirmation' }
                                        }));
                                    }
                                })
                                .catch(() => {
                                    window.dispatchEvent(new CustomEvent('toast', {
                                        detail: { type: 'error', message: 'Erreur de confirmation' }
                                    }));
                                });
                            } catch (error) {
                                window.dispatchEvent(new CustomEvent('toast', {
                                    detail: { type: 'warning', message: 'Paiement réussi mais confirmation en attente' }
                                }));
                            }
                        }
                        ...
                        function onFailed(error) {
                            ...
                            window.dispatchEvent(new CustomEvent('toast', {
                                detail: { type: 'error', message: 'Paiement échoué' }
                            }));
                        }

                        function onCancelled() {
                            window.dispatchEvent(new CustomEvent('toast', {
                                detail: { type: 'info', message: 'Paiement annulé' }
                            }));
                        }
                        ...
                        if (!opened) {
                            window.dispatchEvent(new CustomEvent('toast', {
                                detail: { type: 'error', message: 'Widget de paiement indisponible' }
                            }));
                        }
                    } catch (error) {
