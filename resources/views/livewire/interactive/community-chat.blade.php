...
<div class="creative-light-bg">
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 py-8">
        ...
        <div class="mb-8">
            ...
            <div>
                <a href="{{ route('interactive.events.show', ['event' => $event->slug ?? $event->id]) }}" class="creative-back">
        ...
        @if($readOnly)
            <div class="warning-banner" style="background: linear-gradient(135deg, #fef3c7 0%, #fef7cd 100%); border: 1px solid #fcd34d; border-radius: 16px; padding: 1.25rem; margin-bottom: 2rem; display: flex; align-items: center; gap: 1rem;">
                <svg class="warning-icon" style="width: 24px; height: 24px; color: #d97706;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="creative-input">
                <div class="creative-input-wrapper">
                    ...
                    <input
                        ...
                        class="creative-chat-input"
                        @if($readOnly) disabled @endif
                    >
                    <button wire:click="sendMessage" class="dancing-send" @if($readOnly) disabled @endif>
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        mutations.forEach(function(mutation) {
            if (mutation.type === 'childList') {
                mutation.addedNodes.forEach(function(node) {
                    if (node.classList && node.classList.contains('dancing-message')) {
                        // Add extra dance on new messages
                        setTimeout(() => {
                            node.style.animation = 'messageDanceIn 0.6s cubic-bezier(0.4, 0, 0.2, 1), messagePop 0.8s ease-out';
                            setTimeout(() => {
    ...
    const creativeElements = document.querySelectorAll('.creative-chat-container, .creative-back, .dancing-online');
