        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="form-card rounded-2xl border  p-8">
                <form action="{{ route('events.store') }}" method="POST" enctype="multipart/form-data" id="eventForm">
                    @csrf
                    <div id="form-alert" class="hidden mb-6 rounded-xl border p-4 text-sm"></div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="category" id="category" class="form-input">
                                    ...
                                    @foreach ($categories ?? [] as $key => $label)
                                        <option value="{{ $key }}" @selected(old('category') === $key)>{{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('category')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="datetime-local" name="end_date" id="end_date"
                                        value="{{ old('end_date') }}" class="form-input @error('end_date') error @enderror"
                                        required>
                                    @error('end_date')
                                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="form-group" id="capacity-group">
                                    ...
                                    <input type="number" name="capacity" id="capacity" min="1" value=""
                                ...
                                <div class="space-y-4">
                                    <div class="checkbox-card @if (old('is_interactive')) checked @endif">
                                        <label class="flex items-start space-x-3 cursor-pointer">
                                            ...
                                            <div class="flex-1">
                                                <span class="block text-sm font-semibold text-[#1E3A8A]">Activer
                                                    l'expérience interactive</span>
                                                <span class="mt-1 block text-sm text-[#6B7280]">Affiche les onglets
                                                    Votes/Classement sur la page publique</span>
                                            </div>
                                        </label>
                                    </div>
                                        ...
                                        <label class="flex items-start space-x-3 cursor-pointer">
                                            ...
                                            <div class="flex-1">
                                                <span class="block text-sm font-semibold text-[#1E3A8A]">Page interactive
                                                    publique</span>
                                                <span class="mt-1 block text-sm text-[#6B7280]">Consultation sans connexion
                                                    (votes nécessitent connexion)</span>
                                            </div>
                                        </label>
                                    </div>
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div class="form-group">
                                            <label class="form-label">Début de l'expérience interactive
                                                <span class="text-[#6B7280] text-xs font-normal block">Optionnel. Doit être
                                                    compris entre la date de début et la date de fin de l'événement.</span>
                                            </label>
                                            <input type="datetime-local" name="interactive_starts_at"
                                                value="{{ old('interactive_starts_at') }}" class="form-input">
                                        </div>
                                        <div class="form-group">
                                            <label class="form-label">Fin de l'expérience interactive
                                                <span class="text-[#6B7280] text-xs font-normal block">Doit être
                                                    postérieure au début de l'expérience interactive.</span>
                                            </label>
                                            <input type="datetime-local" name="interactive_ends_at"
                                                value="{{ old('interactive_ends_at') }}" class="form-input">
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <!-- Restriction d'âge -->
                            <div class="checkbox-card @if (old('is_restricted_18')) checked @endif"
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Soumission AJAX du formulaire (pas de redirection, conserve les champs)
            ...
            function showAlert(type, message) {
                ...
                alertBox.classList.remove('bg-white', 'border-slate-200', 'text-slate-700', 'bg-emerald-50',
                    'border-emerald-200', 'text-emerald-700', 'bg-red-50', 'border-red-200', 'text-red-700',
                ...
                if (type === 'success') {
                    ...
                    alertBox.classList.add('bg-white', 'border-slate-200', 'text-slate-700', 'dark:bg-neutral-900',
            ...
            function showFieldErrors(errors) {
                ...
                Object.entries(errors).forEach(([name, messages]) => {
                    ...
                    inputs.forEach(input => {
                        ...
                        p.innerHTML =
                            `<svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>${msg}`;
                        // Insérer après l'input (ou après son parent si select/textarea)
                        const target = input.closest('.form-group') || input;
                        // Éviter doublons
                        const exists = target.querySelector('.error-message');
                        if (!exists) target.appendChild(p);
                    });
                });
            }
